...
인류연합국의 지하 거대한 하수도에서 눈을 뜨게 되며, 같은 구역 내의
다른 장소를 도보로 이동하거나 정거장을 이용해 다른 구역으로 갈 수
있습니다. 각 장소는 `zone` 값을 가지고 있어 같은 구역에 속한 장소들끼리
자유롭게 왕래할 수 있으며, 구역 간 이동은 정거장의 `connections` 설정을
따릅니다.
