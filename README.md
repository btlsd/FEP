...
정거장에 있을 때는 이동 메뉴에서 **탑승**을 선택해 다른 국가로 이동할 수 있습니다.
...
`station: true`인 곳에 있을 때는 이동 메뉴에 **탑승** 옵션이 나타납니다.
탑승하려면 인벤토리에 `탑승권` 아이템이 있거나 플레이어에게 해당 자격
플래그가 부여되어 있어야 합니다. `international: true`가 함께 지정된
정거장(예: 공항, 각 국가의 수도)에서만 다른 나라로 이동할 수 있습니다.
